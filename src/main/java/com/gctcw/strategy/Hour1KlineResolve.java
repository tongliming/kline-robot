...
@Slf4j
@AllArgsConstructor
public class Hour1KlineResolve implements KlineStrategy{
    ...
    public void generateData(IeoKlinedata ieoKlinedata) {
        ...
        ieoKlinedata.setCtime(String.valueOf(ctime - MathUtil.remainder(ctime, KlineType.HOUR1)));
        ...
        if (ieoKlinedatas == null) {
            ...
            log.info("Save 1 hour -> {}", ieoKlinedatas);
            ...
            BigDecimal rateDecimal = MathUtil.getRate(ieoKlinedatas.getPrice(), ieoKlinedatas.getClose());
            ieoKlinedatas.setRate(rateDecimal.toPlainString());
            log.info("Update 1 hour -> {}", ieoKlinedatas);
