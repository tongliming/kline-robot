...
@Slf4j
@AllArgsConstructor
public class Min5KlineResolve implements KlineStrategy{
    ...
    public void generateData(IeoKlinedata ieoKlinedata) {
        ...
        ieoKlinedata.setCtime(String.valueOf(ctime - MathUtil.remainder(ctime, KlineType.MIN5)));
        ...
        if (ieoKlinedatas == null) {
            ...
            log.info("Save 5 min -> {}", ieoKlinedatas);
            ...
            BigDecimal rateDecimal = MathUtil.getRate(ieoKlinedatas.getPrice(), ieoKlinedatas.getClose());
            ieoKlinedatas.setRate(rateDecimal.toPlainString());
            log.info("Update 5 min -> {}", ieoKlinedatas);
