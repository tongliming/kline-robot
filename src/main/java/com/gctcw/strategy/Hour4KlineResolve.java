...
@Slf4j
@AllArgsConstructor
public class Hour4KlineResolve implements KlineStrategy{
    ...
    public void generateData(IeoKlinedata ieoKlinedata) {
        ...
        ieoKlinedata.setCtime(String.valueOf(ctime - MathUtil.remainder(ctime, KlineType.HOUR4)));
        ...
        if (ieoKlinedatas == null) {
            ...
            log.info("Save 4 hour -> {}", ieoKlinedatas);
            ...
            BigDecimal rateDecimal = MathUtil.getRate(ieoKlinedatas.getPrice(), ieoKlinedatas.getClose());
            ieoKlinedatas.setRate(rateDecimal.toPlainString());
            log.info("Update 4 hour -> {}", ieoKlinedatas);
