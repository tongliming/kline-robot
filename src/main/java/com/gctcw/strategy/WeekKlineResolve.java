...
@Slf4j
@AllArgsConstructor
public class WeekKlineResolve implements KlineStrategy{
    ...
    public void generateData(IeoKlinedata ieoKlinedata) {
        ...
        ieoKlinedata.setCtime(String.valueOf(ctime - MathUtil.remainder(ctime, KlineType.WEEK)));
        ...
        if (ieoKlinedatas == null) {
            ...
            log.info("Save 1 week -> {}", ieoKlinedatas);
            ...
            BigDecimal rateDecimal = MathUtil.getRate(ieoKlinedatas.getPrice(), ieoKlinedatas.getClose());
            ieoKlinedatas.setRate(rateDecimal.toPlainString());
            log.info("Update 1 week -> {}", ieoKlinedatas);
